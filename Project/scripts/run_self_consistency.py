...
from src.pipeline import load_core_configs, run_self_consistency, save_results
...
def main() -> None:
    ...
    with tee_console_to_file(log_path):
        print(f"[self_consistency] config={args.config}")
        print(f"[self_consistency] log_path={log_path}")
        results = run_self_consistency(run_config, model_config, prompt_config)
        save_results(run_config["output_dir"], result_name, results)
        print(f"Self-Consistency 运行完成，结果已写入：{Path(run_config['output_dir'])}")
